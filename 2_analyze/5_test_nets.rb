...
class CategoryRecognizer
  ...
  attr_accessor :svm, :fann, :keywords

  def initialize
    @svm, @fann = {}, {}
    @keywords = {'traffic' => TRAFFIC_KEYWORDS, 'sport' => SPORT_KEYWORDS, 'gov' => GOV_KEYWORDS}
    load_svm_models('5_trained_models/1_svm/')
    load_fann_models('5_trained_models/2_fann/')
  end

  def load_svm_models(path_to_dir)
    Dir.glob("#{path_to_dir}/*").each do |path|
      category = path.match(/.+_(.+)\./)[1]
      svm[category] = Libsvm::Model.load(path)
    end
  end

  def load_fann_models(path_to_dir)
    Dir.glob("#{path_to_dir}/*").each do |path|
      category = path.match(/.+_(.+)\./)[1]
      fann[category] = RubyFann::Standard.new(filename: path)
    end
  end
  ...
  def compute_matches(normalized_text_content, category)
    keywords_matches = []
    @keywords[category].each do |keyword|
  ...
  def method_missing(message, *args)
    ...
    if category
      ...
      svm_prediction = @svm[category[1]].predict(Libsvm::Node.features(matches)) == 1
      fann_prediction = @fann[category[1]].run(matches)[0].round == 1
      [svm_prediction, fann_prediction]
    else
...
p CategoryRecognizer.new.is_gov?(article)
