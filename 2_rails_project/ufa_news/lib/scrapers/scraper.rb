...
def scrape
  ...
  sources.each do |source_name, source_attributes|
    ...
    rss_items.each do |rss_item|
      ...
      article_title = article.css(source_attributes[:nodes][:title]).text
      article_content = article.css(source_attributes[:nodes][:content]).text
      ...
      #     rank = 1
      #     puts "#{category} #{rank} #{article_content[0..15]}"
